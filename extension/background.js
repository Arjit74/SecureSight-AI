...
// Normalize URLs to match them correctly (handles trailing slashes, www, etc)
function normalizeUrl(url) {
  try {
    const urlObj = new URL(url);
    // Remove trailing slash, normalize www
    const pathname = urlObj.pathname.replace(/\/$/, '') || '/';
    const normalized = `${urlObj.protocol}//${urlObj.hostname}${pathname}${urlObj.search}`;
    return normalized;
  } catch (e) {
    return url;
  }
...
browser.webNavigation.onBeforeNavigate.addListener(async (details) => {
  ...
  const normalizedUrl = normalizeUrl(url);  // Normalize for comparison
  ...
  // === CRITICAL: Check bypass FIRST (memory is fastest) ===
  // Check both original and normalized URL
  if (state.bypassedUrls.has(normalizedUrl) || state.bypassedUrls.has(url)) {
    const expiry = state.bypassedUrls.get(normalizedUrl) || state.bypassedUrls.get(url);
    if (Date.now() < expiry) {
      console.log(`✅ URL recently bypassed (fast check): ${url}`);
      console.log(`   Bypassed set contains: ${Array.from(state.bypassedUrls.keys()).join(', ')}`);
      return;
    } else {
      console.log(`🗑️ Bypass expired for: ${url}`);
      state.bypassedUrls.delete(normalizedUrl);
      state.bypassedUrls.delete(url);
    }
  }
  
  // Check domain bypass (www.example.com or example.com)
  try {
    const urlObj = new URL(url);
    const domainKey = `${urlObj.protocol}//${urlObj.hostname}`;
    if (state.bypassedUrls.has(domainKey)) {
      const expiry = state.bypassedUrls.get(domainKey);
      if (Date.now() < expiry) {
        console.log(`✅ Domain bypassed: ${domainKey}`);
        return;
      } else {
        console.log(`🗑️ Domain bypass expired: ${domainKey}`);
        state.bypassedUrls.delete(domainKey);
      }
    }
  } catch (e) {
    console.warn('Could not check domain bypass:', e.message);
  }
  
  // Check if already allowed (fast path)
  if (state.allowedUrls.has(normalizedUrl) || state.allowedUrls.has(url)) {
    console.log(`✅ URL previously allowed: ${url}`);
    return;
  }
  
  // Prevent duplicate processing of same tab (fast path)
  ...
  if (state.pendingScans.has(normalizedUrl) || state.pendingScans.has(url)) {
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    ...
    case 'PROCEED_WITH_URL':
      handleProceedWithUrl(request, sender, sendResponse);
      return true;
...
async function handleProceedWithUrl(request, sender, sendResponse) {
  const { url } = request;
  const tabId = sender.tab?.id;  // Get tabId from sender, not request
  const normalizedUrl = normalizeUrl(url);
  
  if (!tabId) {
    console.error('❌ No tabId found in message sender');
    sendResponse({ status: 'error', error: 'No tabId provided' });
    return;
  }
  
  try {
    console.log(`✅ User proceeding to URL: ${url} (tabId: ${tabId})`);
    console.log(`   Original: ${url}`);
    console.log(`   Normalized: ${normalizedUrl}`);
    
    // CRITICAL: Set bypass for BOTH original and normalized URLs
    const bypassExpiry = Date.now() + (5 * 60 * 1000);
    state.bypassedUrls.set(normalizedUrl, bypassExpiry);
    state.bypassedUrls.set(url, bypassExpiry);
    
    console.log(`📊 Bypass map after addition:`);
    console.log(`   Keys: ${Array.from(state.bypassedUrls.keys()).join(', ')}`);
    
    // Also add the domain to allowed URLs for future navigation
    try {
      const urlObj = new URL(url);
      const domainKey = `${urlObj.protocol}//${urlObj.hostname}`;
      state.bypassedUrls.set(domainKey, bypassExpiry);
      console.log(`🔓 Domain bypass set: ${domainKey}`);
    } catch (e) {
      console.warn('Could not parse domain:', e.message);
    }
    
    await browser.storage.local.set({
      guardianlink_bypassed_url: normalizedUrl,
      guardianlink_bypassed_url_original: url,
      guardianlink_bypassed_timestamp: Date.now()
    });
    
    console.log(`🔓 Bypass set for: ${url} (normalized: ${normalizedUrl})`);
    console.log(`⏱️ Expires in 5 minutes`);
    
    // Clear all blocked state for this URL and tab
    state.blockedTabs.delete(tabId);
    state.pendingScans.delete(normalizedUrl);
    state.pendingScans.delete(url);
    processingTabs.delete(tabId);
